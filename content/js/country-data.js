/**
 * ZIABRIDGE Shared Country + International Phone Code Dataset
 * Used by: Contact Form (pages/contact.html) and Join Network (pages/register.html)
 *
 * This file only PROVIDES data + small helper functions to populate
 * <select> elements. It does not alter any existing form logic or
 * submission behavior.
 */

const ZIABRIDGE_COUNTRIES = [
...
/**
 * Populate a <select> with the full country name list.
 * @param {HTMLSelectElement} selectEl
 * @param {string} [defaultValue] - country name to pre-select (e.g. "Bangladesh")
 */
function ziabridgePopulateCountrySelect(selectEl, defaultValue) {
  if (!selectEl) return;
  ...
  ZIABRIDGE_COUNTRIES.forEach(function (c) {
    const opt = document.createElement("option");
    opt.textContent = c.name;
    if (defaultValue && c.name === defaultValue) {
      opt.selected = true;
    }
    selectEl.appendChild(opt);
  });
...
/**
 * Populate a <select> with unique international dial codes (deduplicated,
 * sorted numerically), each labeled with the code and representative
 * country name for clarity, e.g. "+880 (Bangladesh)".
 * @param {HTMLSelectElement} selectEl
 * @param {string} [defaultDial] - dial code to pre-select (e.g. "+880")
 */
function ziabridgePopulatePhoneCodeSelect(selectEl, defaultDial) {
  if (!selectEl) return;

  const seen = {};
  const uniqueCodes = [];
  ZIABRIDGE_COUNTRIES.forEach(function (c) {
    if (!c.dial || seen[c.dial]) return;
    seen[c.dial] = true;
    uniqueCodes.push({ dial: c.dial, name: c.name });
  });

  uniqueCodes.sort(function (a, b) {
    return parseInt(a.dial.replace("+", ""), 10) - parseInt(b.dial.replace("+", ""), 10);
  });

  selectEl.innerHTML = "";
  uniqueCodes.forEach(function (c) {
    const opt = document.createElement("option");
    opt.value = c.dial;
    opt.textContent = c.dial + " (" + c.name + ")";
    if (defaultDial && c.dial === defaultDial) {
      opt.selected = true;
    }
    selectEl.appendChild(opt);
  });
