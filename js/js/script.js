...
// Language Dropdown Toggle
  ...
  const langNativeList = document.getElementById("langNativeList");

  // Populate the full language list (from js/language-data.js) into the
  // dropdown. Uses event delegation below so this works regardless of
  ...
  if (langToggleBtn && langDropdown) {
    ...
    if (langNativeList) {
      langNativeList.addEventListener("click", function (e) {
        const item = e.target.closest("li[data-lang-code]");
        if (!item) return;
...
// Sign In Modal (Placeholder)
  ...
  // Shown ONLY when the visitor is navigating back (clicking a Home link,
  // or the browser Back button) - never on open, on a timer, or on
  // random exit-intent/scroll. Not mandatory - closable via the X button
  // or by clicking outside it. Ratings are kept separate for: Founder
  // Profile, each of the 6 Service pages, the About section, and the
  // general site (all other pages).
  // ================================================
  (function initWebsiteRatingPopup() {
    ...
    // After the popup is dismissed/submitted, continue whichever
    // navigation (Home link click or browser Back) triggered it.
    function continuePendingNavigation() {
      if (pendingNavUrl) {
        const url = pendingNavUrl;
        pendingNavUrl = null;
        window.location.href = url;
      } else if (window.__ziabridgeBackGuardActive) {
        window.__ziabridgeAllowNextPopstate = true;
        history.back();
      }
    }
    ...
    // Trigger: only when the visitor is on their way back (not on open,
    // not on a timer, not on random exit-intent). Covers three ways of
    // "going back": clicking a Home link (header, mobile menu, or
    // breadcrumb Home/Services), or using the browser's own Back button.
    if (!sessionStorage.getItem(context.storageKey)) {
      const homeLinks = document.querySelectorAll(
        'a.home-btn[href$="index.html"], a.mobile-nav-link[href$="index.html"], .hero-breadcrumb a[href*="index.html"]'
      );
      ...
      // Browser Back button: push one extra history entry so the first
      // Back press is caught here (popup shown, page does not actually
      // change yet). Submitting/closing the popup calls history.back()
      // again to complete the real navigation.
      if (!window.__ziabridgeBackGuardActive) {
        window.__ziabridgeBackGuardActive = true;
        window.__ziabridgeAllowNextPopstate = false;
        history.pushState({ ziabridgeGuard: true }, "", window.location.href);
      }
      window.addEventListener("popstate", function () {
        if (window.__ziabridgeAllowNextPopstate) {
          window.__ziabridgeAllowNextPopstate = false;
          return;
        }
        if (sessionStorage.getItem(context.storageKey)) return;
        // Re-add the guard entry so this still works if the popup is
        // dismissed and the user tries Back again later.
        history.pushState({ ziabridgeGuard: true }, "", window.location.href);
        triggerShowPopup();
      });
    }
